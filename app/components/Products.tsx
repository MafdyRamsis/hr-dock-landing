...
function BentoPreview({ kind, ar }: { kind: number; ar: boolean }) {
  ...
  if (kind === 1) return <div className="mt-7 max-w-xs rounded-xl border border-slate-200 bg-white/85 p-3 shadow-sm" aria-hidden="true"><span className="block text-[9px] font-semibold text-slate-700">{ar ? "مراجعة مدخلات الرواتب" : "Payroll input review"}</span><div className="mt-3 flex gap-2">{(ar ? ["الحضور", "الإجازات", "الإضافي"] : ["Time", "Leave", "Overtime"]).map((label) => <span key={label} className="rounded-md bg-indigo-50 px-2 py-1 text-[9px] font-medium text-indigo-700">{label}</span>)}</div></div>;
...
const highlights = [
  { name: ["A first day with fewer loose ends", "بداية عمل دون مهام معلقة"], detail: ["Bring the employee profile, required documents and onboarding steps into one process.", "اجمع ملف الموظف والمستندات المطلوبة وخطوات التهيئة في مسار واحد."], module: 2, className: "md:col-span-7", icon: "M5 20v-2a4 4 0 0 1 4-4h6a4 4 0 0 1 4 4v2M12 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8z" },
  { name: ["Time data you can review", "بيانات وقت جاهزة للمراجعة"], detail: ["See shifts, attendance, leave and overtime together before the payroll cut-off.", "راجع الورديات والحضور والإجازات والإضافي معاً قبل إغلاق الرواتب."], module: 1, className: "md:col-span-5", icon: "M12 6v6l4 2M12 22a10 10 0 1 0 0-20 10 10 0 0 0 0 20z" },
  { name: ["The right record, when needed", "الملف المناسب وقت الحاجة"], detail: ["Keep contracts, letters and employee documents alongside the people they belong to.", "احتفظ بالعقود والخطابات ومستندات الموظف بجانب ملفه."], module: 2, className: "md:col-span-5", icon: "M7 3h7l4 4v14H7a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2zM14 3v5h4M9 13h6m-6 4h6" },
  { name: ["Answers without rebuilding reports", "إجابات دون إعادة بناء التقارير"], detail: ["Give HR and managers a clearer view of attendance and workforce activity.", "امنح الموارد البشرية والمديرين رؤية أوضح للحضور ونشاط الفرق."], module: 1, className: "md:col-span-7", icon: "M4 20V4m0 16h16M8 16v-5m4 5V7m4 9v-8" },
] as const;
    ...
    <section id="products" className="bg-slate-50 py-24 text-slate-900 md:py-32">
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="mt-20 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
          <div><p className="text-sm font-bold uppercase tracking-[.18em] text-indigo-600">{lang === "ar" ? "الوحدات" : "Explore the modules"}</p><h3 className="mt-3 text-3xl font-bold tracking-tight">{lang === "ar" ? "ابدأ من نقطة التعطّل، ثم اربط بقية العمل" : "Start at the bottleneck. Connect the rest."}</h3></div>
        ...
        <div className="mt-24">
          ...
          <div className="mt-10 space-y-12">
            {([
              { kind: "payroll", number: "01", name: ["Payroll & attendance", "الرواتب والحضور"], text: ["See the inputs behind the pay run—attendance, leave, shifts and adjustments—before they turn into last-minute questions.", "راجع مدخلات الرواتب—الحضور والإجازات والورديات والتعديلات—قبل أن تتحول إلى أسئلة في اللحظة الأخيرة."], points: [["Attendance & shifts", "الحضور والورديات"], ["Leave to payroll", "الإجازات والرواتب"], ["Configurable Egypt settings", "إعدادات قابلة للتهيئة لمصر"]] },
              { kind: "operations", number: "02", name: ["HR operations", "عمليات الموارد البشرية"], text: ["Keep employee requests, documents and lifecycle steps out of disconnected inboxes and folders.", "اجمع طلبات الموظفين ومستنداتهم وخطوات رحلتهم بعيداً عن البريد والمجلدات المتفرقة."], points: [["Onboarding & exit", "التعيين وإنهاء الخدمة"], ["Documents & letters", "المستندات والخطابات"], ["Requests & workflows", "الطلبات وسير العمل"]] },
              { kind: "hiring", number: "03", name: ["Recruitment & AI", "التوظيف والذكاء الاصطناعي"], text: ["Bring applications, screening and assessments into one track so hiring decisions do not get lost across CVs and messages.", "اجمع طلبات التوظيف والفرز والتقييمات في مسار واحد حتى لا تضيع قرارات التعيين بين السير الذاتية والرسائل."], points: [["Recruitment pipeline", "مراحل التوظيف"], ["Candidate screening", "فرز المرشحين"], ["Assessments", "التقييمات"]] },
