...
export default function Pricing({ content }: { content: SiteContent["pricing"] }) {
  ...
  const hasRates = content.plans.some((plan) => /^\d+(?:\.\d{1,2})?$/.test(plan.monthlyPrice) && /^\d+(?:\.\d{1,2})?$/.test(plan.annualPrice) && plan.monthlyPrice !== plan.annualPrice);
  return (
    <section id="pricing" className="bg-gray-50 py-24">
      <div className="mx-auto max-w-6xl px-6">
        <div className="mx-auto max-w-2xl text-center">
          <p className="text-sm font-bold uppercase tracking-[.18em] text-[#00B4B4]">{content.eyebrow} · Quotes in EGP</p>
          ...
          {hasRates && <div className="mt-8 inline-flex rounded-xl border border-gray-200 bg-gray-100 p-1">
            ...
            <button onClick={() => setAnnual(true)} className={`rounded-lg px-4 py-2 text-sm font-semibold ${annual ? "bg-white text-[#1B2A4A] shadow-sm" : "text-gray-500"}`}>Annual {content.annualDiscount && <span className="text-emerald-600">{content.annualDiscount}</span>}</button>
          </div>}
        </div>
        <div className="mt-14 grid gap-6 lg:grid-cols-3">
          {content.plans.map((plan) => {
            ...
            return <article key={plan.name} className={`relative rounded-2xl p-8 shadow-xl ${plan.featured ? "border-2 border-[#00B4B4] bg-white" : "border border-gray-200 bg-white"}`}>
              ...
              <div className="mt-7 flex flex-wrap items-end gap-1"><span className="text-4xl font-extrabold text-[#1B2A4A]">{/^\d+(?:\.\d{1,2})?$/.test(price) ? `${content.currency} ${Number(price).toLocaleString("en-EG")}` : price}</span>{/^\d+(?:\.\d{1,2})?$/.test(price) ? <span className="pb-1 text-sm text-gray-500">/ employee / mo</span> : <span className="w-full text-xs font-medium text-gray-500">Your quote will be in EGP</span>}</div>
