...
export default function AdminEditor({ initialAuthenticated, initialContent }: { initialAuthenticated: boolean; initialContent: SiteContent | null }) {
  ...
  return <main className="min-h-screen bg-slate-100 pb-28">
    ...
    <div className="mx-auto max-w-6xl space-y-6 px-5 py-8">
      ...
      <section className={card}><div className="mb-6"><p className="text-xs font-bold uppercase tracking-wider text-indigo-600">Commercial</p><h2 className="mt-1 text-2xl font-bold">Pricing</h2></div><div className="grid gap-5 md:grid-cols-2"><Field label="Eyebrow" value={content.pricing.eyebrow} onChange={(v) => setPricing("eyebrow", v)} /><Field label="Annual discount badge" value={content.pricing.annualDiscount} onChange={(v) => setPricing("annualDiscount", v)} /><Field label="Section title" value={content.pricing.title} onChange={(v) => setPricing("title", v)} /><Field label="Section subtitle" value={content.pricing.subtitle} onChange={(v) => setPricing("subtitle", v)} /></div><div className="mt-7 grid gap-5 lg:grid-cols-3">{content.pricing.plans.map((plan, index) => <div key={index} className="rounded-2xl border border-slate-200 bg-slate-50 p-5"><div className="mb-4 flex items-center justify-between"><h3 className="font-bold">Plan {index + 1}</h3><label className="flex items-center gap-2 text-xs font-semibold text-slate-600"><input type="checkbox" checked={plan.featured} onChange={(e) => updatePlan(index, "featured", e.target.checked)} /> Featured</label></div><div className="space-y-4"><Field label="Name" value={plan.name} onChange={(v) => updatePlan(index, "name", v)} /><Field label="Description" value={plan.description} onChange={(v) => updatePlan(index, "description", v)} multiline /><div className="grid grid-cols-2 gap-3"><Field label="Monthly price" value={plan.monthlyPrice} onChange={(v) => updatePlan(index, "monthlyPrice", v)} /><Field label="Annual price" value={plan.annualPrice} onChange={(v) => updatePlan(index, "annualPrice", v)} /></div><Field label="Button label" value={plan.cta} onChange={(v) => updatePlan(index, "cta", v)} /><Field label="Features (one per line)" value={plan.features.join("\n")} onChange={(v) => updatePlan(index, "features", v.split("\n").filter(Boolean))} multiline /></div></div>)}</div></section>
